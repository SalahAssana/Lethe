    ...
    , data(80*60)
{
    ...
    pixmapLabel = new PixmapLabel();
    layout->addWidget(pixmapLabel, 0, 0, 1, 2);

    value = new QLabel();
    layout->addWidget(value, 2, 0, Qt::AlignCenter);

    temp = new QLabel();
    layout->addWidget(temp, 2, 1, Qt::AlignCenter);

    clearData();

    connect(pixmapLabel, SIGNAL(mouseMoved(QPoint)), this, SLOT(mouseMoved(QPoint)));
    connect(pixmapLabel, SIGNAL(fileDropped(QString)), this, SLOT(newData(QString)));
}
...
void MainWindow::clearData() {
    haveData = false;
    for (int i = 0; i < 80*60; ++i) data[i] = 0;
    QImage image(80, 60, QImage::Format_RGB888);
    image.fill(QColor::fromRgbF(0.0, 0.0, 0.0));
    updatePixmapLabel(image);
    value->setText("");
    temp->setText("");
}

void MainWindow::updatePixmapLabel(QImage rgbData) {
    pixmapLabel->setMouseTracking(true);

    QPixmap pixmap(641, 481);
    QPainter painter(&pixmap);

    if (haveData) {
        painter.drawImage(QRect(0, 0, 640, 480), rgbData, QRect(0, 0, 80, 60));
        painter.setPen(QPen(QColor::fromRgbF(0.0, 0.0, 0.0)));
        for (int y = 0; y <= 480; y += 8) painter.drawLine(0, y, 640, y);
        for (int x = 0; x <= 640; x += 8) painter.drawLine(x, 0, x, 480);
    } else {
        painter.drawRect(0, 0, 641, 481);
        painter.setPen(QPen(QColor::fromRgbF(0.8, 0.2, 0.2)));
        painter.setFont(QFont("Times", 20));
        painter.drawText(QRect(0, 0, 641, 481), Qt::AlignCenter, "Drop .bin file here to view");
    }
    pixmapLabel->setPixmap(pixmap);
}

void MainWindow::mouseMoved(QPoint p) {
    if (!haveData) return;

    QString t = QString::number(data[80*p.y() + p.x()]);
    if (t=="0") t = "---";
    value->setText(QString("(%1,%2): %3").arg(p.x()).arg(p.y()).arg(t));
}
...
void MainWindow::newData(QString filePath) {
    if (filePath.endsWith("-rgb.jpg")) filePath = filePath.left(filePath.length()-8) + "-raw.bin";
    QFile rawFile(filePath);

    if (!filePath.endsWith(".bin") || !rawFile.open(QIODevice::ReadOnly) || (rawFile.size()!=9604 && rawFile.size()!=9606)) {
        clearData();
        return;
    }
    unsigned short cameraTemp = 0, minVal, maxVal;
    QDataStream rawIn(&rawFile);
    bool haveCameraTemp = (rawFile.size()==9606);
    if (haveCameraTemp) {
        rawIn >> cameraTemp;
        temp->setText(QString("Camera Temp: %1K").arg(cameraTemp/100.0, 0, 'f', 2));
    } else
        temp->setText("Camera Temp: ?");

    rawIn >> minVal >> maxVal;
    rawIn.readRawData((char *) &data[0], 2*80*60);

    QImage rgbData(80, 60, QImage::Format_RGB888);
    // Map "data" to rgb values in "rgbData" via the colormap
    int diff = maxVal - minVal + 1;
    for (int y = 0; y < 60; ++y) {
        for (int x = 0; x < 80; ++x) {
            int baseValue = data[80*y + x]; // take input value in [0, 65536)
            int scaledValue = 256*(baseValue - minVal)/diff; // map value to interval [0, 256), and set the pixel to its color value above
            rgbData.setPixel(x, y, qRgb(colormap[3*scaledValue], colormap[3*scaledValue+1], colormap[3*scaledValue+2]));
        }
    }

    haveData = true;
    updatePixmapLabel(rgbData);
}
