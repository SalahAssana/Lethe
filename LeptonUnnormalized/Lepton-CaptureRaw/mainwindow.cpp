...
#if HAVE_LEPTON
    LEP_RESULT result = LEP_OpenPort(0,
                                     LEP_CCI_TWI,
                                     400, /* baud rate, ignored anyway */
                                     &portDesc);
#endif
...
void MainWindow::saveSnapshot() {
    ...
    unsigned short fpaTemperature = -1;
#if HAVE_LEPTON
    LEP_GetSysFpaTemperatureKelvin(&portDesc,
                                   &fpaTemperature);
#endif

    // Raw file format: binary, one word for reported camera temp (Kelvin*100), one for min value, one for max value, then 80*60 words of raw data
    // *NOTE* Qt seems to reverse the byte order for the first three numbers relative to the rest.
    QFile rawFile(QString("%1-raw.bin").arg(snapshotCount));
    rawFile.open(QIODevice::Truncate | QIODevice::ReadWrite);
    QDataStream rawOut(&rawFile);
    rawOut << fpaTemperature << rawMin << rawMax;
    rawOut.writeRawData((char *) &rawData[0], 2*rawData.size());
    rawFile.close();

    // JPG image, top quality
    rgbImage.save(QString("%1-rgb.jpg").arg(snapshotCount), "JPG", 100);
}
