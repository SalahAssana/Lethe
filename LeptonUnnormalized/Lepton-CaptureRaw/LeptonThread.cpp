...
#if HAVE_LEPTON
const char *LeptonThread::device = "/dev/spidev0.1"; // Change to 0.1 if necessary!
unsigned char LeptonThread::mode = 0, LeptonThread::bits = 8;
unsigned int LeptonThread::speed = 24000000;
...
void LeptonThread::run() {    
	...
	/*if (bufferArrayIndex < 50)
	{
	    bufferArray[bufferArrayIndex] = round(ms/1.0e6);
	    bufferArrayIndex++;
	}
	...
	if (bufferArrayIndex == 50)
	{
	   ...
	   bufferArrayIndex = 100;
	}*/

	unsigned short* ptr = rawData.data();
	...
	fwrite(&sec, sizeof(time_t), 1, pFile);

	fwrite(&ms, sizeof(long), 1, pFile);
	...
	//outputFile.open("pixelValues.txt", std::fstream::app);

	//outputFile << "\n" << time(NULL) << "\n";

	//Print Out Pixels

	/*for (int pixel = 0; pixel < rawData.size(); pixel++)
	{
	    outputFile << rawData.at(pixel) << " ";
	    newRow++;
	    if (newRow == 80)
	    {
		outputFile << "\n";
